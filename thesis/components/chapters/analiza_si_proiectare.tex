...
\begin{description}
    \item[NF1 -- Performanță (latență per cadru).] Pipeline-ul principal de procesare a unui cadru, măsurat de la momentul achiziției până la difuzarea rezultatelor către clienți, trebuie să se încadreze sub \textbf{100~ms per cadru}, ceea ce permite atingerea unei rate de aproximativ \textbf{30~FPS} pe fluxul live al camerei principale. Această țintă este derivată din modul de operare paralel al firelor de execuție: detectoarele costisitoare (SlowFast, YOLO) rulează în fire dedicate, astfel încât latența percepută să fie dominată de cel mai lent detector de pe calea critică (recunoaștere facială și plăcuțe), în timp ce detectoarele costisitoare sunt suprapuse asincron și nu se adună la latența per cadru.
...
Firul \texttt{HARProcessingThread} este cel mai costisitor: acumulează cadre într-un buffer intern și, când are clipul complet de 64 cadre la \texttt{clip\_duration\_sec=2.0}, rulează SlowFast-R50 cu pathway-urile slow (8 cadre) și fast (32 cadre) la \texttt{crop\_size=224}, clasificând clipul în \texttt{normal}, \texttt{fight} sau \texttt{vandalism}. Datorită costului ridicat și a inerent batched-ness-ului SlowFast-ului, firul tinde să fie cel care impune ritmul throughput-ului HAR -- de aceea este izolat și nu blochează celelalte detectoare.
