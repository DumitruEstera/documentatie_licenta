...
\begin{lstlisting}[caption={Strategia de matching facial (\texttt{\_match\_embedding} din \texttt{facial\_recognition\_system.py}).},label={lst:matching_facial}]
def match_embedding(embedding):                 # vector L2-normalizat, 512-d
    ...
    if not raw:
        return None                             # index gol, niciun candidat

    person_id, distance = raw[0]                # cel mai apropiat vecin (distanta L2)

    if distance >= recognition_threshold:       # 1.0  (echivalent cos_sim = 0.5)
        return None                             # fata necunoscuta

    # Vectori L2-normalizati: L2^2 = 2 - 2*cos_sim  =>  cos_sim = 1 - L2^2/2
    cos_sim = 1.0 - distance ** 2 / 2.0
    confidence = max(0.0, min(1.0, cos_sim))    # marginit la [0, 1]
...
\begin{lstlisting}[caption={Votarea temporală a plăcuțelor (\texttt{PlateTracker.update} din \texttt{license\_plate\_recognition\_system.py}).},label={lst:plate_vote}]
def update(bbox, text, conf):
    ...
    if best_id is None:                          # niciun track: creeaza unul nou
        best_id = next_id()
        tracks[best_id] = {"bbox": bbox, "age": 0,
                           "readings": deque(maxlen=HISTORY),   # ultimele 10 citiri
                           "emitted": None, "emitted_conf": 0.0}

    t = tracks[best_id]
    t["bbox"], t["age"] = bbox, 0                 # reseteaza contorul de inactivitate
    ...
    if len(t["readings"]) >= MIN_VOTES:
        ...
        for txt, c in t["readings"]:
            scores[txt] += c                      # cumuleaza scorurile de confidenta
        best_text, total_conf = scores.most_common(1)[0]
        votes = sum(1 for txt, _ in t["readings"] if txt == best_text)
        if votes >= MIN_VOTES:
            t["emitted"] = best_text              # rezultat consolidat (stabilizat)
            t["emitted_conf"] = total_conf / votes  # media confidentei sustinatoare
...
\begin{lstlisting}[caption={Cascada celor cinci filtre aplicată fiecărui candidat (\texttt{process\_frame} din \texttt{fire\_detection\_system.py}).},label={lst:fire_pipeline}]
for box in yolo_results.boxes:                  # candidati YOLOv10
    ...
    confirmed = consecutive >= MIN_CONSECUTIVE_FRAMES   # 6 cadre consecutive
...
\begin{lstlisting}[caption={Corelarea temporală a detecțiilor prin IoU (\texttt{\_update\_frame\_history} din \texttt{fire\_detection\_system.py}).},label={lst:fire_history}]
def update_frame_history(class_name, bbox, matched_keys):
    ...
    if best_key is not None and best_iou >= IOU_MATCH_THRESHOLD:   # 0.3
        frame_history[best_key]["count"] += 1                      # streak++
...
\begin{lstlisting}[caption={Derivarea indicatorilor \texttt{confirmed} și \texttt{alert} (\texttt{process\_frame} din \texttt{fire\_detection\_system.py}).},label={lst:fire_confirm}]
...
if confirmed:
    ...
    if last is None or (now - last).total_seconds() > ALERT_COOLDOWN:   # 3 s
        should_alert = True
        last_alert_time[alert_key] = now

detection["confirmed"] = confirmed   # eveniment real: se deseneaza pe cadru
detection["alert"] = should_alert    # momentul de declansare a alarmei
...
\begin{lstlisting}[caption={Construcția perechii de intrări slow/fast (\texttt{pack\_frames\_slowfast} din \texttt{data/dataset.py}).},label={lst:slowfast_pack}]
...
def pack_frames_slowfast(frames, num_slow=8, num_fast=32, crop_size=224):
    ...
    slow_idx = np.linspace(0, T_total - 1, num_slow, dtype=np.int64)   # 8 cadre
    fast_idx = np.linspace(0, T_total - 1, num_fast, dtype=np.int64)   # 32 cadre
    slow_tensor = frames_to_tensor(frames[slow_idx], crop_size)
    fast_tensor = frames_to_tensor(frames[fast_idx], crop_size)
    return [slow_tensor, fast_tensor]   # (3, 8, 224, 224) si (3, 32, 224, 224)

def frames_to_tensor(frames, crop_size):
    imgs = []
    for f in frames:                                  # fiecare cadru (H, W, 3)
        img = to_tensor(f).float() / 255.0            # (3, H, W), valori in [0, 1]
        img = resize(img, [crop_size, crop_size])     # decupare spatiala 224x224
        imgs.append(img)
    tensor = torch.stack(imgs, dim=1)                 # (3, T, 224, 224)
...
\begin{lstlisting}[caption={Ajustarea fină în două faze a modelului SlowFast (\texttt{train.py}).},label={lst:two_phase}]
# Faza 1: backbone inghetat -- se antreneaza doar capul de clasificare
model.freeze_backbone()                          # requires_grad = False pe backbone
optimizer = AdamW(
    filter(lambda p: p.requires_grad, model.parameters()),   # doar parametrii activi
    lr=LR, weight_decay=WEIGHT_DECAY,
)
for epoch in range(1, FREEZE_EPOCHS + 1):        # primele 5 epoci
    train_one_epoch(model, train_loader, criterion, optimizer)
    metrics = validate(model, val_loader, criterion)
    save_best_and_last(model, metrics)           # checkpoint pe cel mai bun macro-F1

# Faza 2: ajustare fina completa, cu rate de invatare diferentiate
model.unfreeze_backbone()
param_groups = model.get_optimizer_param_groups(
    lr=LR, lr_backbone_factor=0.1,               # backbone: LR de 10x mai mic decat capul
)
optimizer = AdamW(param_groups, weight_decay=WEIGHT_DECAY)
scheduler = StepLR(optimizer, step_size=LR_STEP_SIZE, gamma=LR_GAMMA)  # 10x la 10 epoci
...
for epoch in range(FREEZE_EPOCHS + 1, EPOCHS + 1):
    ...
    if early_stopping(metrics["macro_f1"]):      # oprire timpurie pe macro-F1
        break
\end{lstlisting}
...
\begin{lstlisting}[caption={Firul de fuziune și politica \emph{ultimul cadru disponibil} (\texttt{merging\_thread} din \texttt{app.py}).},label={lst:merging}]
...
while True:
    ...
    drain(raw_frame_queue, into=pending["frames"])         # (camera_id, frame)
    ...
    for frame_id in list(pending["frames"]):
        timeout = is_frame_too_old(frame_id, max_age=60)   # asamblare fortata
        ...
        if not (face_enabled or plate_enabled):
            ready = True
        if not ready:
            continue                                       # mai asteapta acest cadru
        ...
        create_alarms_and_logs(camera_id, final_frame)     # vezi logica de alarmare
        broadcast_frame(camera_id, final_frame)            # catre clientii WebSocket
\end{lstlisting}
