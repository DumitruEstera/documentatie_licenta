...
\inlinecode{w600k\_mbf} este identică în ambele pachete, astfel încât embeddings-urile
de 512 dimensiuni rămân compatibile, însă \inlinecode{buffalo\_s} include și capul
\inlinecode{genderage}, indispensabil pentru analiza demografică
(secțiunea~\ref{sec:analiza_demografica}). Fără acest cap, estimările de vârstă și gen
ar fi returnate ca \inlinecode{None}.

Pentru fiecare cadru, metoda \inlinecode{process\_frame()} apelează
\inlinecode{self.face\_app.get(frame)}, care efectuează într-o singură trecere
...
\paragraph{Configurarea execuției pe GPU.}
Inferența InsightFace rulează pe GPU prin runtime-ul ONNX, providerul
\inlinecode{CUDAExecutionProvider} fiind configurat explicit cu opțiunea
\inlinecode{cudnn\_conv\_algo\_search="HEURISTIC"}. Această opțiune este o decizie
de implementare importantă în contextul arhitecturii multi-threaded a aplicației:
...
\paragraph{Gestionarea bibliotecilor CUDA și a TensorFlow.}
...
astfel încât \inlinecode{onnxruntime} să le poată găsi fără a necesita setarea
manuală a variabilei \inlinecode{LD\_LIBRARY\_PATH}. În plus, TensorFlow (folosit
...
provoacă conflicte de context CUDA cu \inlinecode{onnxruntime-gpu} și segmentation
fault la inițializare.

\paragraph{Înregistrarea persoanelor.}
Embeddings-urile persoanelor cunoscute sunt generate în momentul înregistrării.
Metoda \inlinecode{register\_person()} primește o listă de imagini faciale, rulează
InsightFace pe fiecare imagine (selectând, în cazul detecției mai multor fețe, pe cea
cu aria bounding box-ului cea mai mare prin \inlinecode{\_extract\_single\_face()}),
extrage embedding-ul de 512 de dimensiuni și îl persistă atât în baza de date, cât și
în indexul FAISS. Pentru încărcarea de imagini prin interfața web,
metoda \inlinecode{extract\_embedding\_from\_image()} acceptă doar imaginile care
conțin exact o singură față detectabilă, evitând astfel asocierea ambiguă a unui
embedding cu o persoană.
...
de embeddings este suficient de mic încât căutarea liniară să rămână instantanee.

\paragraph{Maparea identităților.}
...
multe embeddings, câte unul pentru fiecare imagine de înregistrare).

\paragraph{Persistența și reconstrucția indexului.}
Indexul FAISS este o structură în memorie; sursa de adevăr a embeddings-urilor este
baza de date PostgreSQL. În tabelul \inlinecode{face\_embeddings}, fiecare embedding
este stocat în coloana \inlinecode{embedding} de tip \inlinecode{BYTEA}, serializat cu
\inlinecode{pickle}. La pornirea sistemului, metoda \inlinecode{\_load\_embeddings\_from\_db()}
preia toate embeddings-urile prin \inlinecode{get\_all\_embeddings()} și reconstruiește
indexul de la zero apelând \inlinecode{rebuild\_index()}. Această reconstrucție este
declanșată din nou ori de câte ori se înregistrează, se editează sau se șterge o
persoană, asigurând consistența între baza de date și index. Metoda de încărcare
include și o protecție împotriva embeddings-urilor reziduale: dacă primul vector are o
dimensiune diferită de 512 (de exemplu, embeddings de 128 de dimensiuni dintr-un
pipeline FaceNet anterior), încărcarea este abandonată, deoarece acei vectori sunt
incompatibili cu modelul curent.

\paragraph{Siguranța accesului concurent.}
...
\inlinecode{rebuild\_index()} apelează intern \inlinecode{add\_embeddings\_batch()},
...
\paragraph{De la distanța L2 la similaritatea cosinus.}
...
\inlinecode{nume (încredere)}, iar cele necunoscute cu un chenar portocaliu și eticheta
...
O optimizare deliberată a pipeline-ului facial constă în condiționarea analizei
demografice de rezultatul recunoașterii. În \inlinecode{process\_frame()}, atributele
demografice -- vârsta și genul (produse de capul \inlinecode{genderage} al InsightFace)
și emoția (produsă de rețeaua Mini-Xception, descrisă în
secțiunea~\ref{sec:analiza_demografica}) -- sunt calculate \textit{exclusiv pentru
fețele necunoscute}. Pentru o față recunoscută, câmpurile \inlinecode{age},
\inlinecode{gender}, \inlinecode{emotion} și \inlinecode{emotion\_confidence} sunt
setate direct la \inlinecode{None}.

Raționamentul are atât o componentă de proiectare, cât și una de performanță. Din
punct de vedere semantic, pentru o persoană deja identificată informațiile demografice
sunt redundante: identitatea, vârsta înregistrată și departamentul sunt deja cunoscute
din baza de date. Din punct de vedere al performanței, omiterea evită o trecere
suplimentară prin rețeaua de clasificare a emoțiilor pentru fiecare față cunoscută, în
fiecare cadru -- o economie semnificativă, având în vedere că această inferență rulează
pe CPU. Astfel, costul analizei demografice este plătit doar atunci când aduce
informație utilă, adică pentru persoanele care nu pot fi identificate și pentru care
caracterizarea demografică și emoțională devine relevantă în scenariile de
supraveghere.
...
detecție a fețelor. Așa cum s-a arătat în secțiunea~\ref{subsec:optimizare_demografica},
analiza demografică este aplicată condiționat -- numai pentru fețele necunoscute -- pentru
că pentru persoanele identificate aceste atribute sunt redundante.

Sursa atributelor este împărțită între două modele: vârsta și genul provin de la capul
\inlinecode{genderage} al InsightFace, deja inclus în trecerea \inlinecode{buffalo\_s},
iar emoția este produsă de o rețea convoluțională separată, Mini-Xception, rulată pe
crop-ul facial. La nivelul interfeței, analiza poate fi activată sau dezactivată global
de către administrator prin endpoint-ul \inlinecode{/api/demographics/toggle}, care
comută flag-ul \inlinecode{demographics\_enabled}.

\subsection{Estimarea vârstei și genului}
\label{subsec:varsta_gen}

Vârsta și genul nu necesită un model dedicat: ambele sunt produse de capul
...
de InsightFace, fiind doar citite și formatate. Tocmai disponibilitatea capului
\inlinecode{genderage} a justificat alegerea variantei \inlinecode{buffalo\_s} în locul
variantei \inlinecode{buffalo\_sc}, care nu îl include
...
Mini-Xception, o rețea neuronală convoluțională compactă livrată de pachetul Python
\inlinecode{fer} prin fișierul de greutăți \inlinecode{emotion\_model.hdf5}. Modelul este
...
Predicția propriu-zisă, implementată în metoda \inlinecode{\_classify\_emotion()}, constă
într-o trecere înainte prin rețea, urmată de \inlinecode{argmax} peste cele șapte ieșiri
softmax; rezultatul este perechea (etichetă emoție, scor de încredere). Dimensiunea de
intrare așteptată de model este citită automat din \inlinecode{input\_shape}, astfel încât
preprocesarea (descrisă în continuare) se adaptează la geometria modelului.
...
Înainte de a fi trecut prin rețeaua de emoții, crop-ul facial (extras din cadru pe baza
bounding box-ului InsightFace, în format BGR) parcurge un lanț de preprocesare care
reproduce fidel pipeline-ul intern al pachetului \inlinecode{fer}, astfel încât greutățile
antrenate să primească intrări în distribuția pe care o așteaptă. Pașii, implementați în
\inlinecode{\_classify\_emotion()}, sunt următorii:

\begin{enumerate}
    \item \textbf{Conversia în tonuri de gri} -- crop-ul BGR este convertit în grayscale
    cu \inlinecode{cv2.cvtColor}, deoarece modelul Mini-Xception operează pe un singur canal;
    \item \textbf{Redimensionarea} -- imaginea grayscale este redimensionată la dimensiunea
    de intrare a modelului (\inlinecode{\_emotion\_target\_size}, dedusă din
    \inlinecode{input\_shape});
    \item \textbf{Normalizarea} -- valorile pixelilor sunt scalate din intervalul $[0; 255]$
    în $[0; 1]$ (împărțire la 255), apoi mapate în intervalul $[-1; 1]$ prin transformarea
    $(x - 0{,}5) \cdot 2$;
    \item \textbf{Adăugarea dimensiunilor de tensor} -- matricea este extinsă la forma
    \inlinecode{(1, H, W, 1)} (dimensiunea de batch și canalul), formatul așteptat de un
    model Keras.
\end{enumerate}
...
CPU este minor, iar GPU-ul rămâne dedicat integral modelelor grele (InsightFace și cele
patru detectoare YOLO/SlowFast care rulează concurent). Repartizarea modelelor pe
dispozitive -- modelele grele pe GPU, clasificatorul ușor de emoții pe CPU -- reflectă astfel
o decizie explicită de echilibrare a încărcării în arhitectura multi-threaded a aplicației.
...
din fișierul \inlinecode{license\_plate\_recognition\_system.py} și rulează într-un fir de
execuție dedicat (\textit{Thread~3}).

Pipeline-ul combină un detector de obiecte (YOLOv8) cu un motor de recunoaștere optică a
caracterelor (EasyOCR), între care se interpun mai mulți pași de preprocesare a imaginii și,
ulterior, o etapă de stabilizare temporală a rezultatului. O provocare centrală, care a
modelat întreaga implementare, este faptul că plăcuțele apar în cadru la rezoluții mici
(tipic 116--151~px lățime), insuficiente pentru un OCR fiabil; din acest motiv pipeline-ul
include un pas explicit de mărire (up-scaling) și o preprocesare agresivă. Cele opt
subcapitole care urmează descriu, în ordinea execuției, etapele acestui pipeline.
...
Localizarea plăcuțelor în cadru este realizată de un model YOLOv8 încărcat prin clasa
\inlinecode{YOLO} din pachetul \inlinecode{ultralytics}, din fișierul de greutăți
\inlinecode{models/license\_plate/best.pt}. Modelul este o variantă YOLOv8 pre-antrenată
specific pentru detecția plăcuțelor de înmatriculare (o singură clasă,
\inlinecode{license\_plate}), preluată ca atare; nu a necesitat antrenare suplimentară în
cadrul proiectului. La inițializare, dacă există suport CUDA, modelul este mutat pe GPU.

Pentru fiecare cadru, metoda \inlinecode{process\_frame()} apelează
\inlinecode{predict()} cu \inlinecode{imgsz=640} și un prag de încredere
\inlinecode{conf=0.25}, obținând bounding box-urile candidate. Fiecare cutie
\inlinecode{[x1, y1, x2, y2]} este apoi \textit{expandată} cu 20\% în fiecare direcție prin
funcția \inlinecode{expand\_bbox()} (un \inlinecode{pad\_ratio} de \inlinecode{0.20},
mărit față de valoarea inițială de \inlinecode{0.05}). Acest padding mai generos asigură
că marginile plăcuței și o mică margine de context sunt incluse în decupaj, evitând tăierea
caracterelor de la extremități înainte de OCR. Decupajul rezultat este transmis pașilor
următori.

\subsection{Up-scaling-ul plăcuțelor mici}
\label{subsec:upscaling}

Întrucât plăcuțele detectate sunt frecvent prea mici pentru ca OCR-ul să le citească corect,
decupajul trece prin funcția \inlinecode{upscale\_if\_small()}, care îl mărește la o lățime
țintă de \inlinecode{250}~px. Logica este următoarea: dacă lățimea curentă este sub prag, se
calculează un factor de scalare $\mathit{scale} = 250 / w$, plafonat la un maxim de
\inlinecode{3.0} (pentru a nu amplifica excesiv zgomotul), iar imaginea este redimensionată
cu interpolare bicubică (\inlinecode{cv2.INTER\_CUBIC}), aleasă pentru calitatea
reconstrucției la mărire. Plăcuțele care depășesc deja lățimea țintă sunt lăsate neschimbate.
Acest pas transformă, de exemplu, un decupaj de $116 \times 33$~px într-unul de cca.
$250 \times 70$~px, dimensiune la care OCR-ul devine fiabil.

\subsection{Pipeline-ul de preprocesare pentru OCR}
\label{subsec:preprocesare_ocr}

Decupajul mărit este apoi pregătit pentru OCR de funcția \inlinecode{preprocess\_plate()},
care aplică un lanț de patru operații tunat pentru EasyOCR pe plăcuțe:
...
\begin{enumerate}
    ...
    \inlinecode{tileGridSize=(8, 8)}) -- compensează umbrele și iluminarea neuniformă;
...
O citire dintr-un singur cadru este nesigură: OCR-ul poate confunda caractere similare
(\inlinecode{O}/\inlinecode{0}, \inlinecode{B}/\inlinecode{8}) de la un cadru la altul. Pentru
a obține un rezultat stabil, fiecare detecție este urmărită între cadre de clasa
\inlinecode{PlateTracker}. Asocierea unei detecții cu un traseu existent (\textit{track}) se
face pe baza suprapunerii spațiale: se calculează \textit{IoU} (\textit{Intersection over
Union}, prin funcția \inlinecode{\_iou()}) între bounding box-ul curent și cele ale traseelor
active, iar detecția este atribuită traseului cu IoU maxim, dacă acesta depășește pragul
\inlinecode{iou\_threshold=0.3}. Dacă niciun traseu nu se potrivește, se creează unul nou, cu
un identificator unic.

Fiecare traseu păstrează un istoric de citiri recente într-o coadă mărginită
(\inlinecode{deque} cu lungimea \inlinecode{history=10}) și un câmp \inlinecode{age} care
numără de câte cadre nu a mai fost actualizat. La fiecare cadru, metoda
\inlinecode{age\_tracks()} incrementează vârsta tuturor traseelor și le elimină pe cele care
depășesc \inlinecode{max\_age=15} cadre, astfel încât vehiculele care au părăsit scena să nu
mai fie urmărite.

\subsection{Validarea formatului}
\label{subsec:validare_format}

Înainte ca o citire să fie luată în considerare pentru votare, ea este validată față de
formatul plăcuțelor românești printr-o expresie regulată,
\inlinecode{ROMANIAN\_PLATE\_RE}. Aceasta acceptă 1--2 litere de județ, urmate de 2--3 cifre
și 3 litere (de exemplu, \inlinecode{CJ12ABC} sau, în cazul Bucureștiului,
\inlinecode{B123ABC}). În metoda \inlinecode{PlateTracker.update()}, o citire este adăugată
în istoricul de voturi al traseului numai dacă este nevidă, are încrederea cel puțin
\inlinecode{0.4} \textit{și} respectă acest format. Citirile malformate (rezultate, de
regulă, din erori de OCR) sunt astfel filtrate înainte de a ajunge în vot, ceea ce crește
robustețea rezultatului final.

\subsection{Votarea temporală}
\label{subsec:votare_temporala}

Rezultatul stabil al unui traseu este decis printr-un \textit{vot majoritar} pe istoricul de
citiri valide. Logica, implementată tot în \inlinecode{PlateTracker.update()}, se activează
numai după ce traseul a acumulat cel puțin \inlinecode{min\_votes=3} citiri. Voturile sunt
agregate într-un \inlinecode{Counter} în care fiecare text candidat acumulează suma
încrederilor citirilor sale; textul cu scorul total maxim este selectat, iar dacă numărul de
citiri care îl susțin atinge pragul \inlinecode{min\_votes}, el devine rezultatul ``emis'' al
traseului (\inlinecode{emitted}), cu o încredere egală cu media încrederilor voturilor sale.

Doar după ce un traseu produce o citire stabilizată sistemul efectuează căutarea în baza de
date: \inlinecode{lookup\_owner\_by\_plate()} caută proprietarul, iar prezența unei intrări
marchează vehiculul ca autorizat. Plăcuțele nestabilizate sau necunoscute sunt raportate ca
\inlinecode{UNKNOWN} / \inlinecode{"Unknown car"}, iar vizual vehiculele autorizate sunt
încadrate cu verde, cele neautorizate cu roșu. Acest mecanism de votare temporală asigură că o
plăcuță este declarată și verificată doar atunci când mai multe cadre consecutive sunt de
acord asupra aceluiași text valid, eliminând citirile tranzitorii eronate.

\section{Modulul de detecție foc și fum}
\label{sec:modul_foc_fum}

Modulul de detecție a focului și a fumului identifică în fluxul video apariția flăcărilor și
a fumului, declanșând alerte timpurii în scenariile de incendiu. Este implementat în clasa
\inlinecode{FireDetectionSystem} din fișierul \inlinecode{fire\_detection\_system.py} și
rulează într-un fir de execuție dedicat (\textit{Thread~4}).

Provocarea principală a acestui modul nu este detecția în sine, ci controlul falselor
pozitive: focul și mai ales fumul sunt clase notoriu de predispuse la confuzii (pereți gri,
nori, abur, reflexii calde, apusuri de soare). O detecție brută a unui model YOLO, folosită
direct ca alarmă, ar genera un volum inacceptabil de alarme false. Din acest motiv,
implementarea adaugă peste model un \textit{pipeline de filtrare în cinci niveluri}, care
transformă detecțiile candidate în alerte numai după ce trec, în ordine, prin toate filtrele.
Subcapitolele care urmează descriu modelul de bază, cele cinci niveluri de filtrare și, în
final, semnificația flag-urilor \inlinecode{confirmed} și \inlinecode{alert} atașate fiecărei
detecții.
...
Modelul de bază este \inlinecode{TommyNgx/YOLOv10-Fire-and-Smoke-Detection}, o variantă
YOLOv10 publicată pe HuggingFace și antrenată specific pentru cele două clase de interes:
\inlinecode{Fire} și \inlinecode{Smoke}. Modelul este folosit ca atare, fără antrenare
suplimentară, fiind încărcat prin clasa \inlinecode{YOLO} din pachetul \inlinecode{ultralytics}
din fișierul de greutăți \inlinecode{models/fire\_and\_smoke/best.pt}. Numele claselor sunt
normalizate la litere mici (\inlinecode{fire}, \inlinecode{smoke}) imediat după încărcare,
pentru a fi folosite consistent în restul pipeline-ului.

La fiecare cadru, metoda \inlinecode{process\_frame()} rulează \inlinecode{model.predict()} cu
un prag de încredere egal cu \textit{cel mai mic} dintre pragurile per clasă (și un IoU de NMS
de \inlinecode{0.45}). Această alegere este deliberată: modelul este lăsat să returneze toți
candidații peste pragul minim, iar filtrarea fină per clasă este aplicată ulterior, explicit,
de pipeline -- ceea ce dă control complet asupra deciziei de alertă.
...
\paragraph{Formatul intrării (contractul SlowFast).}
...
\paragraph{Integrarea în aplicație.}
